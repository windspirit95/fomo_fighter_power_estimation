...
# ============================================================================
# TAB 2: MEMBER STATS MANAGER UI
# ============================================================================

def render_member_stats():
    """Render the member stats manager interface"""
    ...
    with st.sidebar:
            ...
            try:
                ...
                if not isinstance(import_data, dict):
                    ...
                    import_secret = st.text_input(
                        ...
                        type="password",
                        key="import_secret_input_tab2"
                    )
                    
                    col_import, col_cancel = st.columns(2)
                    
                    with col_import:
                        if st.button("✅ Import Data", type="primary", key="import_btn_tab2"):
                    ...
                    with col_cancel:
                        if st.button("❌ Cancel Import", key="cancel_import_btn_tab2"):
                            st.rerun()
        ...
        if st.session_state.get('show_import_options', False):
            ...
            import_mode = st.radio(
                ...
                key="import_mode_radio_tab2"
            )
            
            col_confirm, col_cancel = st.columns(2)
            
            with col_confirm:
                if st.button("Confirm Import", type="primary", key="confirm_import_btn_tab2"):
            ...
            with col_cancel:
                if st.button("Cancel", key="cancel_import_options_btn_tab2"):
                    st.session_state.show_import_options = False
                    st.session_state.pop('import_data', None)
                    st.rerun()
        ...
        if st.button("🗑️ Clear All Data", type="secondary", key="clear_all_data_tab2"):
        ...
        if st.session_state.get('show_clear_dialog', False):
            ...
            clear_secret_input = st.text_input(
                "Enter secret key:", 
                type="password",
                key="clear_secret_input_tab2"
            )
            
            if st.button("Confirm Clear All", type="primary", key="confirm_clear_btn_tab2"):
            ...
            if st.button("Cancel", key="cancel_clear_btn_tab2"):
                st.session_state.show_clear_dialog = False
                st.rerun()
...
# Create tabs
tab1, tab2 = st.tabs(["🎯 Power Calculator", "📊 Member Stats"])
